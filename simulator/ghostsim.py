...
a single arm of the instrument. The key default parameters are in the __init__ function
for the ARM.
"""
...
class ARM():
	...
	def __init__(self,arm):
		"""Initialise an arm of the spectrograph (e.g. red or blue)
		
		Parameters
		----------
		arm: string
			A string representing the arm in use. For GHOST, this is
			"red" or "blue"
		"""
		...
		if (arm == 'red'):
			...
			self.theta_cwl = 20.0  #Grism angle of indicence.
	...
	def simulate_image(self,x,w,b,matrices,im_slit,spectrum=[],nx=0, xshift=0.0, yshift=0.0, rv=0.0):
			...
			if len(spectrum)==0:
				d =pyfits.getdata('ardata.fits.gz')
			...
			for i in range(nm):
				for j in range(ny):
					...
					xy_scaled = np.dot( matrices[0,0], np.array([cutout_xy[0][ww]+int(the_x)-the_x,cutout_xy[1][ww]])/self.microns_pix ).astype(int)
...
#A code block to simulate blue camera data...		
if (True):
	xshift = 0.3
	yshift = 0.1
	rv = 1100.0
	rv_thar = 100.0
	flux = 1e2 #photons/pix for star
	rnoise = 3.0
	
	arm = ARM('blue')
	#arm = ARM('red')

	x,w,b,matrices = arm.spectral_format_with_matrix()

	slit_fluxes = np.ones(19)*0.37
	slit_fluxes[6:13] = 0.78
	slit_fluxes[9] = 1.0
	slit_fluxes /= np.mean(slit_fluxes)
	im_slit = arm.make_lenslets(fluxes=slit_fluxes, mode='high', llet_offset=2)
	image1 = arm.simulate_image(x,w,b,matrices,im_slit, xshift=xshift,rv=rv)

	thar = np.loadtxt('mnras0378-0221-SD1.txt',usecols=[0,1,2])
	...
	im_slit2 = arm.make_lenslets(fluxes=slit_fluxes, mode='high', llet_offset=0)
	image2 = arm.simulate_image(x,w,b,matrices,im_slit2, spectrum=thar_spect, xshift=xshift,rv=rv_thar)

	image = np.maximum(image1+image2,0)
	image = np.random.poisson(flux*image) + rnoise*np.random.normal(size=image.shape)
	hdu = pyfits.PrimaryHDU(image)
	hdu.writeto('test_rv_blue.fits', clobber=True)

#plt.clf()
#plt.imshow(image**0.5, interpolation='nearest', cmap=cm.gray)